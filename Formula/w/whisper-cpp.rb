class WhisperCpp < Formula
  ...
  def install
    ...
    system "cmake", "-B", prefix, *args, *std_cmake_args
    # the "main" target is our whisper-cpp binary
    system "cmake", "--build", prefix, "--target", "main"

    # only the binary and dylib files are needed; delete the rest
    cleanup_files = Dir[prefix/"**/*"].reject do |f|
      f == (prefix/"bin/main").to_s || f.end_with?(".dylib") || File.directory?(f)
    end
    rm cleanup_files
    rmdir Dir[prefix/"**/*"].select { |f| Dir.empty?(f) }
    mv prefix/"bin/main", prefix/"bin/whisper-cpp"

    mkdir prefix/"test"
    mv ["models/for-tests-ggml-tiny.bin", "samples/jfk.wav"], prefix/"test"
  end
  ...
  test do
    system bin/"whisper-cpp", "-m", prefix/"test/for-tests-ggml-tiny.bin", prefix/"test/jfk.wav"
